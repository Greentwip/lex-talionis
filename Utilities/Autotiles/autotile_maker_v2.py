...
print('Reading files %s...' %(len(autotiles)))
books = []
for fp in autotiles:
    image = Image.open(fp)
...
main_fp = 'MapSprite.png'
main = Image.open(main_fp)

new_images = [Image.new('RGB', main.size) for n in range(16)]
print(main.size)
...
for x in range(main.size[0]/WIDTH):
    for y in range(main.size[1]/HEIGHT):
        ...
        print(min_sim)
        if closest_series:
            color_change_band(closest_series, closest_palette, current, test_im, (x, y))

print('Saving...')
for idx, n in enumerate(new_images):
    n.save('autotile' + str(idx) + '.png')
