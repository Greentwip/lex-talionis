...
# === TILE ALGORITHMS ===
    ...
    def getStealables(self):
        if cf.CONSTANTS['steal'] == 0:
            return [item for item in self.items if not item.weapon and not item.spell and not item.locked]
        else:
            return [item for item in self.items if item is not self.getMainWeapon() and not item.locked]
