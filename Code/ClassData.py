...
def create_class_dict():
    ...
    for klass in GC.CLASSDATA.getroot().findall('class'):
        ...
        maximum = Utility.intify_comma_list(klass.find('max').text) if klass.find('max') is not None else [60]
        desc = (klass.find('desc').text) if klass.find('desc') is not None else ''
        ...
        class_dict[c_id] = {'short_name': short_name,
                            'long_name': long_name,
                            'id': c_id,
                            'tier': tier,
                            'wexp_gain': wexp_gain,
                            ...
                            'exp_when_attacked': exp_when_attacked,
                            'max_level': max_level}
